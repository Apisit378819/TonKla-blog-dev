  ...
  const copyToClipboard = () => {
    ...
    navigator.clipboard
      ...
      .then(() => {
        ...
        // รีเซ็ต `copied` ให้เป็น false หลังจาก 2 วินาที
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get(
        ...
        `https://ton-kla-blog-dev-sever.vercel.app/posts/${param.id}`
      );
  ...
  return (
    <>
      ...
      <div className=" container lg:px-8 mx-auto">
        <img
          src={dataPostpage.image}
          alt=""
          className=" w-full object-cover h-[500px]"
        />
        <div className=" flex flex-col px-4 py-4">
          <div className="w-14 h-8 px-3 py-1 rounded-full bg-[#D7F2E9]">
            <p className="text-[#12B279] font-medium">
              {dataPostpage.category}
            </p>
          </div>
